<template>
  <main>
    ...
    <div :class="['bio', { hide: hideBio }]">
      <h2 class="title spacing">
        Howdy!
        <span title="Hide biography" @click="hideBio = true">
...
export default Vue.extend({
  ...
  mounted() {
    document.querySelectorAll('a[href^="https"]').forEach(link => {
      ...
      ;(link as HTMLAnchorElement).rel = 'noreferrer noopener'
    })
  },
  watch: {
    hideBio: {
      handler(v: boolean) {
        document.body.classList.toggle('hide-overflow-y', v)
      },
      immediate: true
    }
  }
